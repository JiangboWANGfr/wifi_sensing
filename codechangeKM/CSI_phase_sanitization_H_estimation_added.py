...
# Use larger delta_t values for fast testing after changes 202508
delta_t = 1e-6           # faster than 1e-7
delta_t_refined = 1e-8   # faster than 5e-9
...
if __name__ == '__main__':
    ...
    for name in names:
        ...
        print(f"[DEBUG] signal_complete shape: {signal_complete.shape}")
        print(f"[DEBUG] signal preview (stream 0, packet 0): {signal_complete[:, 0, 0]}")
        ...
        subcarriers_space = 2
        # delta_t = 1E-7 before changed 202508
        # delta_t_refined = 5E-9 before changed 202508
        ...
        for stream in range(0, 4):
            ...
            # for time_step in range(end_r - start_r): before changed 202508

            # after changed 202508 from here
            total_steps = end_r - start_r
            time_step_start = time.time()

            for time_step in range(total_steps):
                step_elapsed = time.time() - time_step_start
                percent = ((time_step + 1) / total_steps) * 100
                est_total = step_elapsed / (time_step + 1) * total_steps
                est_remaining = est_total - step_elapsed

                print(f"    [Progress] Step {time_step+1}/{total_steps} "
                      f"({percent:.1f}%) | Elapsed: {step_elapsed:.1f}s | "
                      f"Remaining: {est_remaining:.1f}s", end='\r')
                signal_time = signal_considered[:, time_step]   # after changed 202508 to here
                ...
                complex_opt_r = lasso_regression_osqp_fast(signal_time, T_matrix, select_subcarriers, row_T, col_T,
                                                           ...
                                                           zeros_nm_matr)
                #from here changed 202508
                if complex_opt_r.size == 0:
                  print(f"    [⚠️] Skipping empty result at time_step {time_step}")
                  continue
                position_max_r = np.argmax(abs(complex_opt_r)) # to here changed 202508
    
                # position_max_r = np.argmax(abs(complex_opt_r)) before changed 202508
            ...
            print()  # To ensure the last progress message is flushed properly changed 202508
            ...
            stream_elapsed = time.time() - stream_start_time #changed 202508
            print(f"[✅] Finished stream {stream} in {stream_elapsed:.2f} seconds.\n") #changed 202508
