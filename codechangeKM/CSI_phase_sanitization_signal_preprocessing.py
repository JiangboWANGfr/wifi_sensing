...
import os
os.makedirs("phase_processing", exist_ok=True)
# To avoid this problem in the future, add this line before the loop:


for name in names:
    name_file = os.path.join('phase_processing', f"signal_{name}.txt")
    if path.exists(name_file):
        print(f"[INFO] Already processed: {name_file}")
        continue

    csi_buff_file = os.path.join(exp_dir, name + ".mat")  # ✅ FIXED path logic
    print(f"[DEBUG] Loading: {csi_buff_file}")            # ✅ Optional debug print

    csi_buff = sio.loadmat(csi_buff_file)                 # ✅ Load AFTER path is fixed
    ...
    #     # csi_buff = sio.loadmat(csi_buff_file) before changed 202508
    #     print(f"[DEBUG] Loading: {csi_buff_file}")            # ✅ Optional debug print

    #     csi_buff = sio.loadmat(csi_buff_file) #after changed 202508

    #     import os
    #     csi_buff_file = os.path.join(exp_dir, name + ".mat") # after changed 202508
