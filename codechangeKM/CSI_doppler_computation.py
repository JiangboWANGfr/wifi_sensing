...
if __name__ == '__main__':
    ...
    for subdir in list_subdir.split(','):
        #path_doppler = args.dir_doppler + subdir # before changed 202508
        #path_doppler = args.dir_doppler + subdir #after changed 202508
        path_doppler = os.path.join(args.dir_doppler, subdir) #after changed 202508 one more for windows

        if not os.path.exists(path_doppler):
            #os.mkdir(path_doppler) before changed 202508
            os.makedirs(path_doppler, exist_ok=True)


        #exp_dir = args.dir + subdir + '/' before changed 202508
        exp_dir = os.path.join(args.dir, str(subdir)) #after changed 202508
        ...
        for name in names:
            # path_doppler_name = path_doppler + '/' + name + '.txt' # before changed 202508
            path_doppler_name = os.path.join(path_doppler, name + '.txt') # after changed 202508

            if os.path.exists(path_doppler_name):
                continue

            print(path_doppler_name)
            # name_file = exp_dir + name + '.mat' # before changed 202508
            # path_doppler_name = path_doppler + '/' + name + '.txt' #after changed 202508 removed after
           
            name_file = os.path.join(exp_dir, name + '.mat') #after changed 202508
            ...
            if csi_d_profile_array.ndim < 2 or csi_d_profile_array.shape[0] == 0:
               print(f"[WARNING] Empty Doppler profile for {name_file}. Skipping...")
               continue    # change 202508, from if to continue
    ...
    print(f"[✅] Saved: {path_doppler_name}")
